...
# This script calculates the PRS for each sampled individual and calculates
# the level of divergence in PRS using the ANOVA F-stat
...
f_stats <- c()
p_values <- c()
# loops through each trait, calculates a PRS for the sampled individuals,
...
for (code in codes) {
  ...
  for (chr in 1:22) {
    ...
    #print(paste("Loaded encodings for chromosome",chr))
  }
  
  dps <- c()
  # loops through each individual in the sample and computes their PRS
...
# saves the PRS for all sampled individuals and all traits to a file
