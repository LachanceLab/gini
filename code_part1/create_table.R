...
for (i in 1:nrow(traits_table)) {
  ...
  n_cases <- slice$n_cases_full_cohort_both_sexes
  ...
  # calculates overall allele frequency for binary trait GWAS
  if (!("af_EUR" %in% colnames(sf))) {
    sf <- sf %>%
      mutate(af_EUR = (n_cases*af_cases_EUR + n_controls*af_controls_EUR)/(n_total))
  }
  # filters to SNPs with known EUR allele frequencies
  sf <- sf %>% filter(!is.na(af_EUR))
  #sf <- sf %>% filter(!is.na(af_EUR), chr != "X", chr != "Y")
  n_sig_SNPs_EUR <- nrow(sf)
  traits_table[i, "n_sig_SNPs_EUR"] <- n_sig_SNPs_EUR
  ...
  if ((threshold < nrow(sf)) | !(threshold_zero_padding)) {
    gvc_list <- (sf %>% filter(rank <= threshold))$gvc
  } else {
